...
<xsl:stylesheet version="3.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:hei="https://digi.ub.uni-heidelberg.de/schema/tei/heiEDITIONS" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:map="http://www.w3.org/2005/xpath-functions/map">
	<!-- 
    aim:
    ...
    logic: 
          - the variable $element_map provides a map of element names and corresponding heiEDITIONS Concepts resources
          - the semantic-logical container elements whose names are listed in $element_map are resolved to a <milestone> and 
...
-->
	<xsl:param name="element_map" select="map{
    'ab' : 'hc:Chunk',
    'address' : 'hc:Address',
	...
	<xsl:template name="handle-attributes">
        <xsl:param name="elid" as="xs:string" select="''"/>
		<xsl:copy-of select="@* except @ana"/>
		<xsl:choose>
			<xsl:when test="@ana">
				<xsl:attribute name="ana" select="$element_map(local-name()) || ' ' || @ana"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="ana" select="$element_map(local-name())"/>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:if test="not(@corresp) and $elid != ''">
			<xsl:attribute name="corresp" select="'#' || $elid"/>
		</xsl:if>
	</xsl:template>

	<!-- transform all elements whose names are included in $element_map above as keys and which are not members of hc:EditorialContent -->
	<xsl:template match="*[map:contains($element_map, local-name())][ancestor-or-self::text][not(ancestor-or-self::*[tokenize(@ana, '\s+') = ('hc:EditorialContent', 'hc:FigureDescription')])]">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="local-name() = ('persName', 'placeName', 'orgName', 'term', 'title', 'rs', 'name')">
						<xsl:element name="milestone" namespace="http://www.tei-c.org/ns/1.0">
							<xsl:call-template name="handle-attributes">
                                <xsl:with-param name="elid" select="$id"/>
                            </xsl:call-template>
						</xsl:element>
					</xsl:when>
					<xsl:when test="local-name() = ('date', 'docDate')">
						<xsl:element name="milestone" namespace="http://www.tei-c.org/ns/1.0">
							...
							<xsl:attribute name="ana" select="$element_map(local-name())"></xsl:attribute>
					...
					<xsl:when test="local-name() = 'l'">
						<xsl:element name="milestone" namespace="http://www.tei-c.org/ns/1.0">
							...
							<xsl:choose>
								<xsl:when test="@ana">
									<xsl:attribute name="ana" select="$element_map(local-name()) || ' ' || @ana"></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="ana" select="$element_map(local-name())"></xsl:attribute>
								</xsl:otherwise>
